...
class HttpTransport(Transport):
    """Custom HTTP transport for GCH Frappe Sentry integration."""
...
def get_sentry_dsn() -> str:
    """Get the Sentry DSN from the Frappe database."""
    sentry_dsn = frappe.conf.get("sentry_dsn")
    if not sentry_dsn:
        sentry_dsn = frappe.db.get_single_value("GCH Sentry Settings", "sentry_dsn")
    return sentry_dsn
