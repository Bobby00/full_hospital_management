...
class ErpSmartController:
    ...
    def fetch_visits(current_patient):
        """ """
        ...
        try:
            ...
            if code == "200":
                if not response["content"]:
                    frappe.log_error(
                        response,
                        "INFO: No visits found for patient",
                    )
                    return "Smart Member has no details registered"

                result = response["content"][0]

                member_exists = frappe.db.exists("Smart Auth", result["member_number"])

                if member_exists:
                    updated_smart_auth = update_smart_auth_doc(
                        result, current_patient.name
                    )
                    return {
                        "code": 200,
                        "message": "Smart Member details updated",
                        "content": updated_smart_auth,
                    }
                ...
                return {
                    "code": 200,
                    "message": "Smart Member details fetched",
                    "content": doc,
                }

            return {
                "code": 200,
                "message": "Smart Member has no details registered",
                "content": response,
            }
            ...
            return {
                "code": 500,
                "message": "Error fetching Smart Member details",
                "content": e,
            }

    @frappe.whitelist(allow_guest=True)
    def merge_visit_to_session(current_patient, current_encounter):
        try:
            ...
            if response.get("status"):
                return (
                    "Error merging visit to session: Manually Add visit to Smart Auth"
                )
    ...
    @frappe.whitelist(allow_guest=True)
    def fetch_member_details(current_patient, current_encounter):
        try:
            ...
            frappe.log_error(e, "SMART FETCH MEMBER DETAILS ERROR  /erp_smart.py")
            return {
                "code": 500,
                "message": "Error fetching Smart Member details",
                "content": e,
    ...
    def upload_claim(
        ...
        try:
            ...
            if float(amount) < (
                float(current_invoice_doc.total_insured_amount)
                - float(current_invoice_doc.total_advance)
            ):
            ...
            invoiced_customer = current_invoice_doc.customer
            member_number = frappe.db.get_value(
                "Smart Auth", {"visit_number": current_encounter_doc.name}, ["name"]
            )
            current_smart_auth_doc = frappe.get_doc("Smart Auth", member_number)

            diagnosis = get_invoice_diagnosis(current_encounter_doc.name)

            if len(diagnosis) <= 0:

                frappe.log_error(
                    f"No diagnosis in the encounter: {current_encounter_doc.name} ",
                    "SMART UPLOAD CLAIM  ERROR  /erp_smart.py",
                )

                return {
                    "code": 400,
                    "message": "Error uploading claim",
                    "content": f"Kindly add diagnosis to the Encounter: {current_encounter_doc.name}",
                }
            ...
            items = get_insurance_line_items(current_invoice_doc.name)

            if len(items) <= 0:
                frappe.log_error(
                    f"No items in the invoice: {current_invoice_doc.name} ",
                    "SMART UPLOAD CLAIM  ERROR  /erp_smart.py",
                )
                return {
                    "code": 400,
                    "message": "Error uploading claim",
                    "content": f"Kindly add items to the invoice: {current_invoice_doc.name}",
                }
            ...
            advanced_payments = get_advance_payment_list(current_invoice_doc.name)
                ...
                payload_modifiers.append(
                    {
                        ...
                        "reference_number": current_invoice_doc.name,
                    }
                )
            ...
            payload = {
                "claim_code": current_invoice_doc.name,
                "payer_code": current_smart_auth_doc.payer_code,
                "payer_name": current_smart_auth_doc.payer_name,
                "medicalaid_code": current_smart_auth_doc.medicalaid_code,
                "amount": current_invoice_doc.outstanding_amount,
                "gross_amount": current_invoice_doc.grand_total,
                "batch_number": "batch4",
                "dispatch_date": str(datetime.datetime.now().replace(microsecond=0)),
                "patient_number": current_patient_doc.uhid_code,
                "patient_name": current_smart_auth_doc.member_name,
                "location_code": location_name,
                "location_name": location_name,
                "scheme_code": current_smart_auth_doc.schemecode,
                "scheme_name": current_smart_auth_doc.scheme_name,
                "member_number": current_smart_auth_doc.name,
                "visit_number": current_encounter_doc.name,
                "session_id": int(current_smart_auth_doc.session_id),
                "visit_start": str(
                    current_encounter_doc.creation.replace(microsecond=0)
                ),
                "visit_end": str(datetime.datetime.now().replace(microsecond=0)),
                "currency": "KES",
                "doctor_name": current_encounter_doc.practitioner_name,
                "sp_id": int(sp_id),
                ...
                "admission": {
                    ...
                    "admission_date": str(
                        current_invoice_doc.creation.replace(microsecond=0)
                    ),
                ...
                "invoices": [
                    {
                        ...
                        "invoice_number": current_invoice_doc.name,
                        "invoice_ref_number": current_invoice_doc.name,
                        "lines": line_items,
                        "payment_modifiers": payload_modifiers,
                        "pool_number": pool_nr,
                        "service_type": current_smart_auth_doc.session_type,
                    }
                ],
            }
            ...
            response = smart.upload_claim(
                current_smart_auth_doc.session_id,
                current_patient_doc.uhid_code,
                payload,
            )

            if response.get("status"):
                frappe.log_error(response, "SMART UPLOAD CLAIM RESPONSE /erp_smart.py")
                return {
                    "code": 500,
                    "message": "Error uploading claim",
                    "content": response,
            ...
            frappe.log_error(response, "here SMART UPLOAD CLAIM RESPONSE /erp_smart.py")
                ...
                new_smart_invoice = frappe.get_doc(
                    {
                        ...
                        "session_id": current_smart_auth_doc.session_id,
                        "sales_invoice": current_invoice_doc.name,
                        "patient_encounter": current_encounter_doc.name,
                        "smart_auth": current_smart_auth_doc.name,
                        "id": id,
                        "total_amount": (
                            float(current_invoice_doc.total_insured_amount)
                            - float(current_invoice_doc.total_advance)
            ...
            frappe.log_error(response, "sm 1 SMART UPLOAD CLAIM RESPONSE /erp_smart.py")
            return {"code": 200, "message": "Success", "content": response}

        except Exception as e:

            frappe.log_error(e, "SMART UPLOAD CLAIM ERROR  /erp_smart.py")
            return {"code": 500, "message": "Error uploading claim", "content": e}
    ...
    def get_smart_benefits_options(encounter: str):
        try:
            ...
            frappe.log_error(e, "SMART CLOSE CLAIM ERROR  /erp_smart.py")
            return []

    @frappe.whitelist(allow_guest=True)
    def close_visit(current_invoice_doc):
        try:
            frappe.log_error(
                f"Closing visit for invoice: {current_invoice_doc.name}",
                "INFO: Closing visit",
            )
            current_smart_invoice_doc = frappe.get_doc(
                "Smart Invoice", current_invoice_doc.name
            )

            smart: Smart = initiate_smart()

            response = smart.close_visit(current_smart_invoice_doc.session_id)

            current_smart_auth_doc = frappe.get_doc(
                "Smart Auth", current_smart_invoice_doc.smart_auth
            )

            if response.get("code") == 200:
                doc = create_payment_entry(
                    current_smart_invoice_doc.total_amount,
                    f"Smart: {current_smart_invoice_doc.id} ({current_smart_auth_doc.scheme_name})",
                    current_smart_invoice_doc.patient,
                )
                return {
                    "code": 200,
                    "message": "Success",
            ...
            return {"code": 500, "message": "Error closing visit", "content": response}

        except Exception as e:

            frappe.log_error(e, "SMART CLOSE CLAIM ERROR  /erp_smart.py")
            return {"code": 500, "message": "Error closing visit", "content": e}
...
def initiate_smart() -> Smart:
    smart_settings = frappe.get_doc("Smart Settings", "Gertrudes")
    ...
    print(base_url, client_id, username, client_secret, password)
    return smart
...
def update_smart_auth_doc(result, patient):
    try:
        ...
        frappe.log_error(e, "SMART UPDATE AUTH ERROR  /erp_smart.py")
        return e
