...
class CENTRALLINEINSERTIONCHECKLIST(Document):
	pass
...
def fetching_triage_information(**args):
    """Fetches the triage information from linked encounter"""
    try:
        # anthro_and_vitals = frappe.db.get_value("Inpatient Record", args["inpatient_record"], ['anthropometry_details', 'vital_signs_table'], as_dict=1)
        anthro_and_vitals = frappe.get_doc("Inpatient Record", args["inpatient_record"])
        
        return anthro_and_vitals
    except Exception as e:
        frappe.log_error(e, "fetching_triage_information /cental_line_insertion_checklist.py")
...
def invoice_procedure_consumables(encounter: str="", procedure: str=""):
    ...
    try:
        ...
        procedure_done =  frappe.get_doc("CENTRAL LINE INSERTION CHECKLIST", procedure)
