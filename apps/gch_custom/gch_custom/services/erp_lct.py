...
class ErpLctController:
    ...
    @frappe.whitelist(allow_guest=True)
    def get_member_details(membership_number: str, patient, encounter):
        """


        """
        try:
            # CHECH IF ENCOUNTER IS INPATIENT OR OUTPATIENT
            is_inpatient = 0
            encounter_key = "encounter"  # Replace with your dynamic key as needed
            ...
            if is_inpatient == 0:
                ...
                current_encounter = frappe.get_doc("Inpatient Encounter", encounter)
    ...
    @frappe.whitelist(allow_guest=True)
    def create_claim(sales_invoice, patient):
        try:
            current_invoice = frappe.get_doc("Sales Invoice", sales_invoice)

            # Check if is inpatient or outpatient
            if current_invoice.encounter is None:
                current_encounter = frappe.get_doc("Inpatient Record", current_invoice.inpatient_record)
            elif current_invoice.encounter is not None:
                current_encounter = frappe.get_doc(
                    "Patient Encounter", current_invoice.encounter)
                
            current_patient = frappe.get_doc(
                "Patient", patient)
            
            # Handling for inpatient and outpatient
            if "INP" in current_encounter:
                membership_number = frappe.db.get_value(
                    "Lct Member", {"inpatient_record": current_encounter.name}, ["name"])
            ...
            for item in item_list:
                lines.append({"billing_code": current_encounter.branch,
                              "charge_date": str(current_invoice.creation),
                ...
                "invoices": [
                    {
                        ...
                        "bill_from":  str(current_invoice.creation),
                        "bill_to":  str(current_invoice.creation),
                        "amount": current_invoice.total_insured_amount,
                        "gross_amount": current_invoice.total,
    ...
    def process_claim(claim):
        try:
            ...
            current_claim = frappe.get_doc("Lct Claim", claim)

            
            if "INP" in current_claim.visit_number:
                current_encounter = frappe.get_doc(
                "Inpatient Record", current_claim.visit_number)
            else:
                current_encounter = frappe.get_doc(
                "Patient Encounter", current_claim.visit_number)

            current_invoice = frappe.get_doc(
                "Sales Invoice", current_encounter.sales_invoice)
            current_patient = frappe.get_doc(
            ...
            for item in item_list:
                lines.append({"billing_code": current_encounter.branch,
                              "charge_date": str(current_invoice.creation),
                              "name": "Out Patient Clinic Consultation",
                              "invoiceline_number": current_invoice.name,
                              "quantity": 1,
                              "unit_price": item["price"]["amount"],
                              "unit_of_measure": "PC",
                              ...
                              "gross_amount": item["price"]["amount"],
                              "category": item["category"],
                              "payment_reference": [],
                              "pre_authorization_code": ""})
            ...
            if exceeded_amount > 0:
                return f"You have exceeded your limit, Please Pay: KES {exceeded_amount}"
