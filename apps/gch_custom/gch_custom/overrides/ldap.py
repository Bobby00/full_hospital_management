...
class GCHLDAPSettings(LDAPSettings):
    def validate(self):
        if not self.enabled:
            return
        if not self.flags.ignore_mandatory:
            ...
            if (
                ...
                try:
                    if conn.result.get("type") == "bindResponse" and self.base_dn:
                        import ldap3
                    ...
                    frappe.throw(
                        _(
                            "LDAP settings are incorrect. validation response was: {0}"
                        ).format(ex),
                    ...
                    frappe.throw(
                        _("Ensure the user and group search paths are correct."),
                        title=_("Misconfigured"),
                    )

                if self.ldap_directory_server.lower() == "custom":
                    if not self.ldap_group_member_attribute:
                        frappe.throw(
                            _(
                ...
                frappe.throw(
                    _(
                        "LDAP Search String must be enclosed in '()' and needs to contain the user placeholder {0}, eg sAMAccountName={0}"
                    )
                )

    def connect_to_ldap(
        self, base_dn: str, password: str, read_only: bool = True
    ) -> Any:
        try:
            import ldap3
            import ssl
    ...
    def get_ldap_client_settings():
        ...
        if ldap.enabled:
            result["enabled"] = True
            result["method"] = "gch_custom.services.ldap_login"
        return result
    ...
    def update_user_fields(cls, user, user_data):
        ...
        updatable_data = {
            key: value for key, value in user_data.items() if key != "email"
        }
    ...
    def sync_roles(self, user, additional_groups=None):
        ...
        matched_roles = {
            r.erpnext_role
            for r in self.ldap_groups
            if r.ldap_group.lower() in lower_groups
        }
    ...
    def fetch_ldap_groups(self, user, conn):
        ...
        if len(conn.entries) >= 1:
            ...
            for group in conn.entries:
                udn = group["distinguishedName"].value
                ou = ",".join(udn.split(",")[1:])
                fetch_ldap_groups.append(ou)
                # for g in group['distinguishedName'].value:

                #     fetch_ldap_groups.append(g)
    ...
    def authenticate(self, username, password):
        ...
        conn = self.connect_to_ldap(
            self.base_dn, self.get_password(raise_exception=False)
        )
        ...
        try:
            ...
            if len(conn.entries) == 1 and conn.entries[0]:
                ...
                if (
                    user.entry_dn
                    and password
                    and conn.rebind(user=user.entry_dn, password=password)
                ):
            ...
            frappe.throw(
                _("Please use a valid LDAP search filter"), title=_("Misconfigured")
            )
    ...
    def reset_password(self, user, password, logout_sessions=False):
        ...
        if conn.search(
            ...
            if conn.entries and conn.entries[0]:
                ...
                frappe.throw(
                    _("No Entry for the User {0} found within LDAP!").format(user)
                )
        else:
    ...
    def convert_ldap_entry_to_dict(self, user_entry):
        ...
        data = {
            ...
            "email": str(
                email.value[0] if isinstance(email.value, list) else email.value
            ),
        ...
        if self.ldap_phone_field:
            try:
                raw_phone = user_entry[self.ldap_phone_field].value
                if raw_phone and "/" in raw_phone:
